...
function nodePathCheck(node: Hierarchy.Node<PLoad, string>, expectScopeId: string, ...expectSegments: string[]): void {
    ...
    suite('build', () => {
        ...
        suite('parsePath', () => {
            ...
            test('child segments extend parent segments; nodePaths are unique', function () {
                ...
                function walk(parent: Hierarchy.Node<PLoad, string> | null, node: Hierarchy.Node<PLoad, string>): void {
        ...
        suite('Lookup', () => {
            ...
            test('finds leaf node by full path', function () {
                ...
                const found = Hierarchy.lookup(topNodes, ['a', 'b', 'c']);
            ...
            test('finds intermediate branch node', function () {
                ...
                const found = Hierarchy.lookup(topNodes, ['a', 'b']);
            ...
            test('finds data+branch node', function () {
                ...
                const found = Hierarchy.lookup(topNodes, ['a']);
                assert.ok(found, 'must find the node');
            ...
            suite('Edges', () => {
                ...
                test('empty segments returns undefined', function () {
                    const topNodes = Hierarchy.build(SCOPE, [spec(['a', 'b'], { tag: 'x' })]);
                    assert.strictEqual(Hierarchy.lookup(topNodes, []), undefined);
                });
                ...
                test('partial match returns undefined', function () {
                    ...
                    const found = Hierarchy.lookup(topNodes, ['a', 'nope']);
                ...
                test('non-existent path returns undefined', function () {
                    ...
                    const found = Hierarchy.lookup(topNodes, ['zzz']);
                    assert.strictEqual(found, undefined);
                });
