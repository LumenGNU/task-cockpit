...
suite('Task.fetch — tasks.json is directory', () => {

    test('tasks.json as directory produces empty maps but scope key is present', async () => {

        // --- Контракт фикстуры ---
        const folders = vscode.workspace.workspaceFolders;
        assert.ok(folders);

        const scope = resolveScopes().find((f) => f.folderName === 'tasks.json-is-directory');
        ...
        try {
            ...
            assert.ok(definitionsByFile.has(scopeFile),
                'definitionsByFile must have scope key even when tasks.json is a directory');
            assert.ok(tasksByFile.has(scopeFile),
                'tasksByFile must have scope key even when tasks.json is a directory');

            // --- Assert: внутренние мапы пусты ---
            assert.strictEqual(definitionsByFile.get(scopeFile)!.size, 0,
                'definitions must be empty when tasks.json is a directory');
            assert.strictEqual(tasksByFile.get(scopeFile)!.size, 0,
                'tasks must be empty when tasks.json is a directory');
