...
async function run(subscriptions, logChannel, traceChannel) {
    subscriptions.push(vscode.tasks.onDidStartTaskProcess((e) => {
        ...
        const indexLnk = tasksList.indexOf(task); // ищем по ссылке
        // @ts-expect-error
        const sameId = tasksList.find((t) => { return t._id === task._id; }); // ищем по id
        const indexId = sameId ? tasksList.indexOf(sameId) : -1; // ищем по id
        ...
        logChannel.appendLine(`  started task: index[${indexLnk >= 0 ? indexLnk + 1 : '?'}]; id[${indexId >= 0 ? indexId + 1 : '?'}] "${task.name}" ` +
            `scope: ${scope} source: ${task.source} command: "${command}"`);
    }));
    ...
    tasksList.forEach((task, index) => {
        ...
        // // ['name', 'source', 'scope', 'definition', 'execution', ...]
        // console.log(getters);
        // Значения
    ...
    //     logChannel.appendLine(' ');
    //     logChannel.appendLine(`Execute task wit index ${(index++)}...`);
