...
suite('Task.fetch — missing file', () => {

    test('absent tasks.json produces empty maps but scope key is present', async () => {

        // --- Контракт фикстуры ---
        const folders = vscode.workspace.workspaceFolders;
        assert.ok(folders);

        const scope = resolveScopes().find((f) => f.folderName === 'missing-file');
        ...
        try {
            ...
            assert.ok(definitionsByFile.has(scopeFile),
                'definitionsByFile must have scope key even without tasks file');
            assert.ok(tasksByFile.has(scopeFile),
                'tasksByFile must have scope key even without tasks file');

            // --- Assert: внутренние мапы пусты ---
            assert.strictEqual(definitionsByFile.get(scopeFile)!.size, 0,
                'definitions must be empty when tasks file is absent');
            assert.strictEqual(tasksByFile.get(scopeFile)!.size, 0,
                'tasks must be empty when tasks file is absent');
