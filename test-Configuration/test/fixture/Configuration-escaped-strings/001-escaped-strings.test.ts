...
suite('escaped-strings (спецсимволы в значениях)', function () {
    ...
    suiteSetup(function () {
        cfg = vscode.workspace.getConfiguration('escapedStrings');
    ...
    test(`${/*++N*/'001'/**/} null byte проходит /^.$/ — возвращает значение`, function () {
        const schema = Configuration.createSchema<{ nullByte: string; }>({
            nullByte: { from: '.', type: OptionType.String, spec: { fallback: 'fallback', pattern: /^(.|fallback)$/ } }
        });
        assert.equal(Configuration.get(schema, cfg).nullByte, '\u0000');
    });

    test(`${/*++N*/'002'/**/} newline не проходит /^.$/ — возвращает фолбек`, function () {
        const schema = Configuration.createSchema<{ newline: string; }>({
            newline: { from: '.', type: OptionType.String, spec: { fallback: 'fallback', pattern: /^(.|fallback)$/ } }
        });
        assert.equal(Configuration.get(schema, cfg).newline, 'fallback');
    });

    test(`${/*++N*/'003'/**/} tab проходит /^.$/ — возвращает значение`, function () {
        const schema = Configuration.createSchema<{ tab: string; }>({
            tab: { from: '.', type: OptionType.String, spec: { fallback: 'fallback', pattern: /^(.|fallback)$/ } }
        });
        assert.equal(Configuration.get(schema, cfg).tab, '\t');
    });

    test(`${/*++N*/'004'/**/} emoji без u-флага не проходит /^.$/ — возвращает фолбек`, function () {
        const schema = Configuration.createSchema<{ emoji: string; }>({
            emoji: { from: '.', type: OptionType.String, spec: { fallback: 'fallback', pattern: /^(.|fallback)$/ } }
        });
        assert.equal(Configuration.get(schema, cfg).emoji, 'fallback');
    });

    test(`${/*++N*/'005'/**/} emoji с u-флагом проходит /^.$/u — возвращает значение`, function () {
        const schema = Configuration.createSchema<{ emoji: string; }>({
            emoji: { from: '.', type: OptionType.String, spec: { fallback: 'fallback', pattern: /^(.|fallback)$/u } }
        });
        assert.equal(Configuration.get(schema, cfg).emoji, '😀');
    });
    ...
    test(`${/*++N*/'006'/**/} combining sequence не проходит /^.$/u — возвращает фолбек`, function () {
        ...
        const schema = Configuration.createSchema<{ combining: string; }>({
            combining: { from: '.', type: OptionType.String, spec: { fallback: 'fallback', pattern: /^(.|fallback)$/u } }
        });
        assert.equal(Configuration.get(schema, cfg).combining, 'fallback');
    });

    test(`${/*++N*/'007'/**/} zero-width space проходит /^.$/ — возвращает значение`, function () {
        const schema = Configuration.createSchema<{ zwsp: string; }>({
            zwsp: { from: '.', type: OptionType.String, spec: { fallback: 'fallback', pattern: /^(.|fallback)$/ } }
        });
        assert.equal(Configuration.get(schema, cfg).zwsp, '\u200B');
    });
    ...
    test(`${/*++N*/'008'/**/} литеральный \\x00 проходит /^\\x00$/ — возвращает значение`, function () {
        const schema = Configuration.createSchema<{ literalEscape: string; }>({
            literalEscape: { from: '.', type: OptionType.String, spec: { fallback: 'fallback', pattern: /^(\\x00$|fallback)/ } }
        });
        assert.equal(Configuration.get(schema, cfg).literalEscape, '\\x00');
    });

    test(`${/*++N*/'009'/**/} одинокий \\ проходит /^.$/ — возвращает значение`, function () {
        const schema = Configuration.createSchema<{ singleBackslash: string; }>({
            singleBackslash: { from: '.', type: OptionType.String, spec: { fallback: 'fallback', pattern: /^(.|fallback)$/ } }
        });
        assert.equal(Configuration.get(schema, cfg).singleBackslash, '\\');
    });
