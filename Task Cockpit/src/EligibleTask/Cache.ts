 ...
 * `Task`-объектов, собранных на момент вычисления. Сам VS Code их
 * постоянно не кэширует — фетчит через `tasks.fetchTasks` по запросу.
...
class Cache implements Disposable {
    ...
    #rPromise: RevocablePromise<Readonly<EligibleTask.Index>> | null = null;
     ...
     *  самого {@linkcode Cache}. Количество итераций цикла равно
    ...
    async get(): Promise<EligibleTask.Index | null> {
    ...
    #scheduleIdleEviction(owner: Promise<Readonly<EligibleTask.Index>>): void {
    ...
    #reScan(): Promise<Readonly<EligibleTask.Index>> {
        ...
        const { promise }
            = this.#rPromise
            = RevocablePromise.runCancellable(fetch);
