...
export class AssertionFailed extends Error {
    readonly taskName = 'AssertionFailed';
...
type AssertFn = (condition: unknown, text: string) => void;
...
function get(file: string): { log: LoggerFn; assert: AssertFn; fail: FailFn; table: TableFn; } {

    const ext = vscode.extensions.getExtension('papio-dev.task-cockpit');
    const id = ext ? `${file.replace(`${ext.extensionPath}${path.sep}`, '')}` : file;

    return {
        log: out.bind(undefined, id),
...
function table(id: string, level: vscode.LogLevel, data: Record<string, unknown> | ReadonlyArray<Record<string, unknown>>, config?: { headers?: string[]; undefinedAsEmpty?: boolean; }) {
    if (!logChannel) {
        return;
    }
