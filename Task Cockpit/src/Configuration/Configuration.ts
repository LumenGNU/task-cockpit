...
function isFieldDef(entry: { [k: string]: unknown; }): entry is { section: string; type: OptionType; spec: object; } {
    return (
...
export function createSchema<S extends object>(schema: Readonly<ConfigSchema<S>>): typeof schema {
    ...
    function walkSchema(entry: unknown, path: string[] = []) {
        ...
        if (isKVObject(entry)) {
            ...
            if (isFieldDef(entry)) {
                assert.ok(entry.section.length,
...
export function read<Schema extends object>(props: {
    ...
    baseSection: string,
    configurationScope: vscode.ConfigurationScope | null | undefined;
}): InferConfigType<Schema> {

    const workspaceConfig = vscode.workspace.getConfiguration(props.baseSection, props.configurationScope);
    ...
    return walkSchema(props.schema) as InferConfigType<Schema>;
};
 ...
 * Если `fieldDef.section` равно '.', поиск идет напрямую по имени ключа в объекте.
 * */
function resolveFieldValue(configKey: string, fieldDef: {
    ...
    const input = workspaceConfig.get(fieldDef.section === '.' ? configKey : `${fieldDef.section}.${configKey}`);
