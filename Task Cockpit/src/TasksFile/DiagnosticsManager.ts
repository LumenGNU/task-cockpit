/** @file TasksFile/DiagnosticsManager.ts */
...
const { log, assert, table } = Logger.get(module.filename);
// #endregion DEBUG
 ...
 * */
export default class DiagnosticsManager implements vscode.Disposable {
    ...
    private workspaceFoldersChangedHandler(event: vscode.WorkspaceFoldersChangeEvent): void {
        ...
        for (const folder of event.added) {
            ...
            this.scheduleCheck(vscode.Uri.joinPath(folder.uri, '.vscode', 'tasks.json') as TC.Uri);
        }
    ...
    private getTasksSources(): TC.Uri[] {

        const taskSources: TC.Uri[] = [];

        if (vscode.workspace.workspaceFile) {
            taskSources.push(vscode.workspace.workspaceFile as TC.Uri);
        }

        if (vscode.workspace.workspaceFolders) {
            for (const folder of vscode.workspace.workspaceFolders) {
                taskSources.push(vscode.Uri.joinPath(folder.uri, '.vscode', 'tasks.json') as TC.Uri);
            }
        }
    ...
    private scheduleCheck(uri: TC.Uri): void {
    ...
    private async performCheck(uri: TC.Uri, ctsToken: vscode.CancellationToken): Promise<vscode.Diagnostic[]> {
