/** @file Runtime/TaskProcessLifecycle.ts */
...
import TaskProcessRegistry from './TaskProcessRegistry';
...
type TaskProcessRegistryView = TaskProcessLifecycle.TaskProcessRegistryView;

declare namespace TaskProcessLifecycle {

    /** Публичный интерфейс реестра только для чтения.
     * Скрывает мутирующие методы оставляя только запросы и события. */
    type TaskProcessRegistryView =
        LifecycleOmitted<Omit<TaskProcessRegistry, 'register' | 'markCompleted' | 'reconcile'>>;
 ...
 * состояния через {@linkcode TaskProcessRegistry.onDidChangeTaskProcesses}.
 *
 ...
 *
 * #### Свойства:
 * - `taskProcessRegistry` — доступ к реестру процессов (только чтение)
 *
 * #### Методы:
 ...
 */
class TaskProcessLifecycle implements Disposable {

    static readonly CONFIGURATION_SECTION = 'Terminals' as const;
    #terminalsConfiguration: WindowSettings.Configuration[typeof TaskProcessLifecycle.CONFIGURATION_SECTION];
    ...
    /** {@link TaskProcessRegistry | Реестр процессов} */
    readonly #taskProcessRegistry: TaskProcessRegistry;
    ...
    constructor(
        ...
        this.#taskProcessRegistry = new TaskProcessRegistry(logOutputChannel);
        this.#processMonitor = new TaskProcessMonitor(this.#resourceProps, logOutputChannel);
        this.#snapshotCollector = new SnapshotCollector(this.#resourceProps, logOutputChannel);
    ...
    /** Деактивирует {@linkcode TaskProcessLifecycle} и освобождает все ресурсы.
    ...
    public get taskProcessRegistry(): TaskProcessRegistryView {
        return this.#taskProcessRegistry;
    }
    ...
    public async getTaskProcessRecords(originKey: OriginKey, taskName: TaskName): Promise<Immutable<Array<TaskProcessRecord>>> {
        ...
        const processStates = this.#taskProcessRegistry.getTaskProcessStates(originKey, taskName);
        ...
        const promises = window.terminals.map(async (terminal) => {
            ...
            const processId = await getTerminalProcessId(terminal, this.#terminalsConfiguration.timeout);
     ...
     * */
    public terminateTaskProcesses(originKey: OriginKey, taskName: TaskName): void {
    ...
    async #processStartedHandler({ execution, processId }: TaskProcessStartEvent): Promise<void> {
        ...
        if (isValidPid(processId)) { // сразу отбрасываем сломанное
            ...
            try {
                const taskOriginInfo = await this.#resourceProps.resourceStateCoordinator.resolveTaskOrigin(execution.task);
                ...
                this.#taskProcessRegistry.register(
    ...
    #processCompletedHandler(completedProcessIds: ReadonlySet<TaskProcessId>) {
        ...
        this.#taskProcessRegistry.markCompleted(
            this.#requestIdCounter.next(),
            completedProcessIds
        );
    ...
    #collectSnapshotHandler(snapshot: Immutable<TerminalProcessesSnapshot>) {
        ...
        this.#taskProcessRegistry.reconcile(snapshot.requestId, new Set(snapshot.terminalProcesses));
    }
    ...
    #changeConfigurationHandler(affectedKeys: WindowSettings.AffectedKeys) {
        if (!affectedKeys.has(TaskProcessLifecycle.CONFIGURATION_SECTION)) {
            return;
        }
        this.#terminalsConfiguration = this.#resourceProps.windowSettings.getConfiguration(TaskProcessLifecycle.CONFIGURATION_SECTION);
    }
...
function sendSIGTERM(pid: TaskProcessId): void {
...
export default TaskProcessLifecycle;
