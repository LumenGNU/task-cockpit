...
import Terminals from 'src/Runtime/Terminals';
import type ProcessId from 'src/type.d/ProcessId';


const scenarios = [
    'New Terminal Infinite Process', // Откроет новый терминал, не завершится, не закроет терминал
...
async function awaitOpenNewTerminal(taskScenario: typeof scenarios[number], ms: number = 10_000): Promise<ProcessId | undefined> {
    ...
    return new Promise((resolve, reject) => {
        ...
        const handler = vscode.tasks.onDidStartTaskProcess((e) => {
            clearTimeout(timeout);
            handler.dispose();
            resolve(e.processId as ProcessId | undefined);
        });
...
async function awaitTerminalsSnapshot(trms: Terminals, timestamp: number, ms: number = 3_000): Promise<Terminals.Snapshot> {
...
suite('@module MainPanel.Runtime:Terminals', () => {
    ...
    suite('Scenario T1', () => {
        ...
        setup(async () => {
            trms = new Terminals({ timeout: 1_000 });
        });
        ...
        test('reconcile captures all running terminal PIDs', async () => {
            ...
            const pids = [
                await awaitOpenNewTerminal('New Terminal Infinite Process'),
                await awaitOpenNewTerminal('New Terminal Infinite Process'),
                await awaitOpenNewTerminal('New Terminal Infinite Process'),
            ] as ProcessId[];
            ...
            assert.strictEqual(snapshot.processIds.has(pids[1]!), true, 'Expected process 1 in snapshot');
            assert.strictEqual(snapshot.processIds.has(pids[2]!), true, 'Expected process 2 in snapshot');
        ...
        test('reconcile excludes closed terminal PID', async () => {
            ...
            const pid = await awaitOpenNewTerminal('New Terminal 3s Auto Close') as ProcessId;
